...
class CrossLlvm < Formula
  ...
  def ver; 'cross'; end # version suffix
  ...
  def install
    ...
    # Install Clang tools
    (share/"#{ver}-clang/tools").install buildpath/'tools/clang/tools/scan-build', buildpath/'tools/clang/tools/scan-view' if build.with? 'clang'

    if python
      # Install llvm python bindings.
      mv buildpath/'bindings/python/llvm', buildpath/"bindings/python/#{ver}-llvm"
    ...
    Dir.glob(install_prefix/'bin/*') do |exec_path|
      ...
      ln_s exec_path, bin/"#{ver}-#{exec_file}"
    end
    ...
    Dir.glob(install_prefix/'share/man/man1/*') do |manpage|
      ...
      ln_s manpage, man1/"#{ver}-#{manpage_base}.1"
    end
  end

  def test
    system "#{bin}/#{ver}-llvm-config", "--version"
  end
  ...
  def caveats
    ...
    if build.with? 'clang'
      clang_tools_path = HOMEBREW_PREFIX/"share/#{ver}-clang"
      s += <<-EOS.undent
    ...
    if build.with? 'libcxx'
      include_path = HOMEBREW_PREFIX/"lib/#{ver}-llvm/lib/c++/v1"
      libs_path = HOMEBREW_PREFIX/"lib/#{ver}-llvm/usr/lib"
      s += <<-EOS.undent

      To link to libc++ built here, please adjust your environment as follow:

        CXX="#{ver}-clang++ -stdlib=libc++"
